...
{        
    if (e.target !== e.currentTarget) {
        ...
        else if(T.id=="GRADE")
        {
            ...
            xmlhttp.send("ID1="+ID1+"&ID2="+ID2.join(',')+"&ID3="+ID3+"&ID4="+ID4);    
        }
        ...
        else if(AR[1]=="H") //title or committee
        {
            var QTY = document.getElementById('QTY'+AR[2]).value; 
...
function SEARCHALL()
{
	var T = document.getElementById('SRCHKW');
 	var QTY = document.getElementById('QTY0').value; 	   
 	for(var i=0;i<QTY;i++)
 	{
    	if(document.getElementById('A1_H_0_'+i))
    	{
	    	var A = document.getElementById('A1_H_0_'+i);
	    	if(A.innerHTML.toLowerCase().indexOf(T.value.toLowerCase())!=-1)
	    	{
		    	A.style.display = "inline-block";	
	    	} 	
	    	else
